...
object Sourcer {
  ...
  def sourceFromFile(path: String): BufferedSource = sourceFromFile(
    new File(path)
  )
  ...
  def newFileNotFoundException(path: String): FileNotFoundException = {
    ...
    val message2 =
      message1 + (if (path.startsWith("~"))
                    ".  Make sure to not use the tilde (~) character in paths in lieu of the home directory."
                  else "")
